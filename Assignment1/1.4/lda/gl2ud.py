...
if not os.path.exists("ldamodel"):
    ...
    data = []
    y = []
    with open("../dataset/data/labelled.json", "r") as f:
        for line in tqdm(f):
            temp = json.loads(line)
            data.append((temp["summary"] + " ") * 3 + temp["reviewText"])
            y.append(temp["overall"] - 1)
    ...
    ldamodel = models.LdaMulticore(corpora, num_topics=10, id2word=dictionary, passes=1, workers=8)
    ...
    data = []
    with open("../dataset/data/unlabelled.json", "r") as f:
        for line in tqdm(f):
            temp = json.loads(line)
            data.append((temp["summary"] + " ") * 3 + temp["reviewText"])
