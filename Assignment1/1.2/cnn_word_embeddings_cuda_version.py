...
class CNN(nn.Module):
    def __init__(self,):
        ...
        self.out1 = nn.Linear(16 * 26, 100)
        self.out2 = nn.Linear(100, 5)
...
if retrain:
    ...
    for each in tqdm(corpus):
        ...
        temp = Counter(temp).most_common(max_length)
...
if retrain:
    ...
    x_dev = torch.from_numpy(np.array(x_dev)).cuda()
    y_dev = torch.from_numpy(np.array(y_dev)).cuda()


if os.path.exists("models/cnn_we_cudav2"):
    print("Loading Model")
    cnn, optimizer, loss_func = torch.load("models/cnn_we_cudav2")
else:
    cnn = CNN()
    cnn.cuda()
...
if retrain:
    for epoch in range(EPOCH):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if step % 80 == 0:
                cnn.eval()
                d_x = Variable(x_dev).cuda()
                d_x = d_x.view(d_x.size(0), d_x.size(1))
                d_y = Variable(y_dev).cuda()
                test_output = cnn(d_x)
                pred_y = torch.max(test_output, 1)[1].data.squeeze()

                accuracy = sum(pred_y == y_dev) / float(y_dev.size(0))
                f_score = f1_score(y_dev.cpu().numpy(), pred_y.cpu().numpy(), average="macro")
                print('Epoch: ', epoch, 'step: ', step, '| train loss: %.4f' % loss.data[0], '| dev accuracy: %.2f' % accuracy, '| f score: %.2f' % f_score)

            cnn.train()
        torch.save((cnn, optimizer, loss_func), "models/cnn_we_cuda")

if not retrain:
    print("Predicting")
    cnn.eval()
    y_pred = []
    for i in tqdm(range(0, len(x_dev), 1000)):
        if i + 1000 <= len(x_dev):
            x = x_dev[i:i+1000]
            y = y_dev[i:i+1000]
        else:
        ...
        d_x = Variable(torch.from_numpy(np.array(x))).cuda()
        d_x = d_x.view(d_x.size(0), d_x.size(1))
        d_y = Variable(torch.from_numpy(np.array(y))).cuda()
        test_output = cnn(d_x)
        pred_y = torch.max(test_output, 1)[1].data.squeeze()
        y_pred += (pred_y.cpu().numpy().tolist())
